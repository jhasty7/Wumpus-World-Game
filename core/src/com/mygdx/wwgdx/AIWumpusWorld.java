...
public class AIWumpusWorld extends ScreenAdapter {

	private static final float WORLD_WIDTH = 656;
	private static final float WORLD_HEIGHT = 656;
	private static final int STENCH_COST = 2;
	private static final int BREEZE_COST = 5;
	private static final int STENCH_BREEZE_COST = 5;
	...
	public AIWumpusWorld(WumpusWorldGame EDT) {
		this.EDT = EDT;
		...
		numberOfStenches = stenchList.size();
	}
	...
	private void draw() {
		...
		aiDecision = HillClimingSearchDecision(landedSquare);
	...
	/*
	 * implement hillClimbing search somehow
	 */

	public AI_DECISION HillClimingSearchDecision(int i) {
		int nextCell = 0;
		int rng;
			...
			if (playerX == mapCell.get(i).getX() && playerY == mapCell.get(i).getY()) {
				...
				if (mapCell.get(i).getBreeze() && mapCell.get(i).getStench()) {
					...
					if (!mapCell.get(i).getVisited()){
						...
						System.out.println("------------" + stenchCounter + "  " + numberOfStenches);
				...
				if( stenchCounter == numberOfStenches){
